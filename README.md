...
- Observer
- Command - общий интерфейс (например очередь заданий), необходимо реализовать востановление преведущего состояние (
  undo/ отмена назад, востановления после сбоя)
- Состояние
...
требует, чтобы высокоуровневые компоненты не зависели от низкоуровневых компонентов; вместо этого и те, и другие должны
...
объекта. Этот объект теперь можно логировать, хранить в истории для отмены, передавать во внешние сервисы и так далее.
...
Внедрение зависимостей — это стиль настройки объекта, при котором поля объекта задаются внешней сущностью. Другими
словами, объекты настраиваются внешними объектами.